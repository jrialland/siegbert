...
static bool sendDatagram(const string &host, int port, const string &msg) {
  ...
  bool sent = 0 == sendto(fd, msg.c_str(), msg.length() + 1, 0,
...
UdpAppender::UdpAppender(const string &host, int port)
...
bool UdpAppender::append(const Logger &logger, const LogMessage &m) {
  // format description :
  // https://github.com/apache/logging-log4j2/blob/master/log4j-core/src/main/resources/Log4j-events.dtd
